...
public class StudentLearningController {
    ...
    public String getLearningPath(@RequestParam(required = false) Long classId, Model model) {
        List<ClassRoom> classes = studentLearningService.getJoinedClasses(demoStudentId);
        ...
        List<LearningNode> nodes = studentLearningService.getLearningNodes(demoStudentId, activeClass.getClassId());
        ...
        for (LearningNode node : nodes) {
            nodeResources.put(node.getNodeId(), studentLearningService.getContentResources(demoStudentId, node.getNodeId()));
        ...
        long totalResourcesCount = nodes.stream()
                .mapToLong(node -> studentLearningService.getContentResources(demoStudentId, node.getNodeId()).size())
                .sum();
        long viewedResourcesCount = studentLearningService.getViewedResourcesCountForClass(demoStudentId, activeClass.getClassId());
    ...
    public String getClassDetail(@PathVariable Long classId, Model model) {
        if (!studentLearningService.isStudentEnrolled(demoStudentId, classId)) {
        ...
        List<LearningNode> nodes = studentLearningService.getLearningNodes(demoStudentId, classId);
        ...
        long totalResourcesCount = nodes.stream()
                .mapToLong(node -> studentLearningService.getContentResources(demoStudentId, node.getNodeId()).size())
                .sum();
        long viewedResourcesCount = studentLearningService.getViewedResourcesCountForClass(demoStudentId, classId);
        int overallProgressValue = 0;
        if (totalResourcesCount > 0) {
            overallProgressValue = (int) ((viewedResourcesCount * 100) / totalResourcesCount);
        }
    ...
    public String getNodeDetail(@PathVariable Long nodeId, Model model) {
        Optional<LearningNode> nodeOpt = studentLearningService.getLearningNode(demoStudentId, nodeId);
        if (nodeOpt.isEmpty()) {
            return "redirect:/student/learning-path";
        }

        LearningNode node = nodeOpt.get();
        List<ContentResource> contents = studentLearningService.getContentResources(demoStudentId, nodeId);
    ...
    public String viewContent(@PathVariable Long contentId) {
        Optional<ContentResource> contentOpt = studentLearningService.getContentResource(demoStudentId, contentId);
        if (contentOpt.isEmpty()) {
            return "redirect:/student/learning-path";
        }

        // Log the activity progress
        studentLearningService.logProgress(demoStudentId, contentId);
